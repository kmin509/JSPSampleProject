...
<body>
	<% 
		...
		//그 사람을 select 하려면 "누굴 선택했는가?" 를 알아야함 (id가 필요)
		...
		PersonInfoDTO personInfo = personInfoDAO.findPersonInfoListById(intId);
	%>
	...
	%>
		...
		<form id = "personForm" action="deletePerson_proc.jsp" method = "post">
			...
			<!-- 화면에 보여주지는 않을건데 폼에 값을 보내야하니까 hidden  -->
	...
	<script>
		document.getElementById('deleteBtn').addEventListener('click',()=>{
			...
			if(confirm('삭제 하시겠습니까?')){
				...
				form.action = 'deletePerson_proc.jsp'; //삭제로 액션을 바꿈
				form.submit();
			}
		});
		...
		document.getElementById('modifyBtn').addEventListener('click',()=>{
				...
				if(confirm('수정 하시겠습니까?')){
					let form = document.getElementById('personForm');
					form.action = 'modifyPerson_proc.jsp'; //수정으로 액션을 바꿈
					form.submit();
				}
		});
	</script>
